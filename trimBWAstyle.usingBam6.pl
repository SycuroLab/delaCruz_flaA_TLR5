...
our($opt_q,$opt_o,$opt_f,$opt_l,$opt_out,$opt_h);
getopts('hq:o:f:l:');
...
$read1_file =~ s/.bam/_trimmed.fastq/;
$read2_file =~ s/.bam/_trimmed.fastq/;
$singletonFile =~ s/.bam/_trimmed.fastq/;

# Modification specifica to our analysis pipeline to output files
# in the TRIM directory rather than the ELC directory
$read1_file =~ s/\/ELC\//\/TRIM\//;
$read2_file =~ s/\/ELC\//\/TRIM\//;
$singletonFile =~ s/\/ELC\//\/TRIM\//;
# Modification specific to our analysis pipeline to add mate pair and
# singleton information to the filepaths
$read1_file =~ s/_bmt_/_R1_bmt_/;
$read2_file =~ s/_bmt_/_R2_bmt_/;
$singletonFile =~ s/_bmt_/_singleton_bmt_/;
my $quality_reads = 0;
my $quality_bases = 0;
...
sub checkPos {
    ...
    $maxArea = 0;
    while ($pos>0 && $area>=0) {
	$area += $opt_q - (ord(substr($q,$pos,1))-$opt_o);
	if ($area > $maxArea) {
	    $maxArea = $area;
	    $maxPos = $pos;
	}
